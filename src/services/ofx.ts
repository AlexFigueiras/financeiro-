  ...
  return withTransaction(async (client) => {
    ...
    for (const t of transacoes) {
      ...
      const regraRes = await client.query<{ categoria_chave: string }>(
        `SELECT categoria_chave FROM regras_categorizacao WHERE $1 ILIKE '%' || termo || '%' LIMIT 1`,
        [t.descricao]
      );
      const categoria = (regraRes.rowCount && regraRes.rowCount > 0) ? regraRes.rows[0].categoria_chave : 'outros';

      const result = await client.query(
        `INSERT INTO transacoes_banco (conta_id, data_transacao, descricao_bruta, valor, hash_ofx, origem, categoria)
         VALUES ($1, $2, $3, $4, $5, 'ofx', $6)
         ON CONFLICT (hash_ofx) DO NOTHING`,
        [contaId, t.data.toISOString(), t.descricao, t.valor, hashOfx(t, contaId), categoria]
      );
